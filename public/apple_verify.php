...
try {
    $pdo = get_db_pdo();
    $stmt = $pdo->prepare('SELECT id, apple_email, name FROM users WHERE apple_id = :apple_id LIMIT 1');
    $stmt->execute([':apple_id' => $sub]);
    $existing = $stmt->fetch();

    if ($existing) {
        $stmt = $pdo->prepare('UPDATE users SET
            apple_email = COALESCE(:apple_email, apple_email),
            apple_email_verified = COALESCE(:apple_email_verified, apple_email_verified),
            apple_is_private_email = COALESCE(:apple_is_private_email, apple_is_private_email),
        ...
        $_SESSION['user'] = [
            'provider' => 'apple',
            'apple_id' => $sub,
            'email' => $email ?: ($existing['apple_email'] ?? null),
